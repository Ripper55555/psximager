...
*/
trackmode_t
cdio_get_track_mode(const CdIo_t *p_cdio, track_t u_track)
{
  if (!p_cdio) return TRACK_FORMAT_ERROR;

  if (p_cdio->op.get_track_mode) {
    return p_cdio->op.get_track_mode (p_cdio->env, u_track);
  } else {
    return TRACK_FORMAT_ERROR;
  }
}
...
/*!
  Return the last sector of the track.
  0 is returned if there is an error.
*/
unsigned int
cdio_get_track_end_sector(const CdIo_t *p_cdio, track_t u_track)
{
  lba_t end_lba;
    if (p_cdio == NULL || p_cdio->op.get_track_end_lba == NULL) {
        return 0;
    }
    end_lba = p_cdio->op.get_track_end_lba(p_cdio->env, u_track);
    if (end_lba == CDIO_INVALID_LBA) {
        return 0;
    }
    return end_lba;
}
