...
#define TOOL_VERSION "PSXInject 2.0.1 Final"


// Print usage information and exit.
static void usage(const char * progname, int exitcode = 0, const string & error = "")
{
	cout << "Usage: " << boost::filesystem::path(progname).filename().generic_string() << " [OPTION...] <input>[.bin/cue] <repl_file_path> <new_file>" << endl;
...
int main(int argc, char ** argv)
{
	...
	try {
		...
		CdIo_t * image = cdio_open(imagePath.generic_string().c_str(), DRIVER_BINCUE);
		if (image == NULL) {
		...
		imagePath.replace_extension(".bin");
		fstream writeImage(imagePath.generic_string(), fstream::in | fstream::out | fstream::binary);

		// Read the new file and inject it
		ifstream file(newFileName.generic_string(), ifstream::in | ifstream::binary);
		if (!file) {
